...
#' @export
pcAlign <- function(x,y,optim=TRUE,subsample=NULL,iterations=10,mc.cores=2, verbose = F) UseMethod("pcAlign")

#' @rdname pcAlign
#' @export
pcAlign.matrix <- function(x, y,optim=TRUE,subsample=NULL,iterations=10, mc.cores=2, verbose = F) {
  if (!missing(y)) {
    ...
    if(verbose){
      ...
      return(list(x = x, trafo = fintrafo))
    } else{
      return(x)
    }
...
#' @rdname pcAlign
#' @export
pcAlign.mesh3d <- function(x,y,optim=TRUE,subsample=NULL,iterations=10,mc.cores=2, verbose = F) {
  xorig <- x
  x <- vert2points(x)
  if(verbose){
    
    tmp <- pcAlign(x,y,optim=optim,subsample=subsample,verbose = T, iterations=iterations,mc.cores=mc.cores)
    
    xorig$vb[1:3,] <- t(tmp$x)
    xorig <- vcgUpdateNormals(xorig)
    return(list(x = xorig, trafo = tmp$trafo))
    
  } else{
    tmpverts <- pcAlign(x,y,optim=optim,subsample=subsample,verbose = T, iterations=iterations,mc.cores=mc.cores)
